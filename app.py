...
# Storage backend (local filesystem or Cloudflare R2)
...
def uploaded_file(filename):
    """Serve user-uploaded images via storage backend (local or R2)."""
...
def save_upload(file_storage) -> str | None:
    ...
    storage.save_filestorage(file_storage, name)
    return name
...
def save_attachments(db, post_id: int, files) -> int:
    ...
    for f in files or []:
        ...
        size = storage.save_filestorage(f, stored)
        db.execute(
...
def _crop_region(doc: "fitz.Document", page_idx: int,
    ...
    storage.save_bytes(pix.tobytes("png"), name)
    return name
